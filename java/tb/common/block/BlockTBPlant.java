...
public class BlockTBPlant extends BlockBush implements IGrowable {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float fX, float fY, float fZ) {
		...
		if (world.getBlockMetadata(x, y, z) >= this.growthStages) {
			ArrayList<ItemStack> dropList = this.getDrops(world, x, y, z, world.getBlockMetadata(x, y, z), 0);
			if (dropList.size() > 0) {
				for (ItemStack stack : dropList) {
					if (stack.getItem() instanceof ItemSeeds && world.rand.nextInt(100) < 95 - 1) {
						continue;
					}
//					EntityItem eItem = new EntityItem(world);
//					eItem.setEntityItemStack(stack);
//					eItem.setPosition(x, y + 0.2, z);
					if (!world.isRemote) {
						//world.spawnEntityInWorld(eItem);
//						IPosition tempPosition = new PositionImpl(x, y, z);
//						BehaviorDefaultDispenseItem.doDispense(world, stack, 1, EnumFacing.UP, tempPosition);
                        ...
                        EntityItem entityitem = new EntityItem(world, (double)((float)x + f), (double)((float)y + f1), (double)((float)z + f2), new ItemStack(stack.getItem(), stack.stackSize, stack.getItemDamage()));
