 ...
 public class BlockSpike
   extends Block
 {
   public static final String[] spikeNames = new String[] { "iron", "iron_bloody", "thaumic", "thaumic_bloody", "void", "void_bloody" };
   ...
   public BlockSpike() {
     super(Material.iron);
     float f = 0.0625F;
     setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 1.0F - f, 1.0F);
   }
   ...
   public void onEntityCollidedWithBlock(World w, int x, int y, int z, Entity collider) {
     ...
     if (meta == 0 || meta == 1) {
       ...
       collider.attackEntityFrom(DamageSource.cactus, 8.0F);
       
       if (meta == 0 && collider instanceof EntityLivingBase && ((EntityLivingBase)collider).getHealth() <= 0.0F)
       {
     ...
     if (meta == 2 || meta == 3) {
       ...
       if (!(collider instanceof net.minecraft.entity.item.EntityItem)) {
         collider.attackEntityFrom(DamageSource.cactus, 14.0F);
       }
       if (meta == 2 && collider instanceof EntityLivingBase && ((EntityLivingBase)collider).getHealth() <= 0.0F)
       {
     ...
     if (meta == 4 || meta == 5) {
       ...
       if (collider instanceof EntityPlayer) {
         ...
         if (boots != null && boots.getItem() instanceof thaumcraft.common.items.armor.ItemVoidArmor) {
           return;
         }
       } 
       if (!(collider instanceof net.minecraft.entity.item.EntityItem) && !(collider instanceof net.minecraft.entity.item.EntityXPOrb))
       {
         if (!w.isRemote) {
           
           FakePlayer fake = new FakePlayer((WorldServer)w, fakeSpikeProfile);
           collider.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)fake), 20.0F);
           fake.setDead();
           fake = null;
         } 
       }
       
       if (meta == 4 && collider instanceof EntityLivingBase && ((EntityLivingBase)collider).getHealth() <= 0.0F)
       {
