...
public class BlockCampfire extends BlockContainer {
	...
	public void randomDisplayTick(World w, int x, int y, int z, Random r) {
		if (w.getBlockMetadata(x, y, z) > 1) {
			...
			w.spawnParticle("flame", x + 0.5D + DummySteele.randomDouble(r) / 4.0D, y, z + 0.5D + DummySteele.randomDouble(r) / 4.0D, 0.0D, 0.04D, 0.0D);
			for (int i = 0; i < 10; i++) {
				w.spawnParticle("smoke", x + 0.5D + DummySteele.randomDouble(r) / 4.0D, y + 0.1D, z + 0.5D + DummySteele.randomDouble(r) / 4.0D, 0.0D, r.nextDouble() / 20.0D, 0.0D);
			}
