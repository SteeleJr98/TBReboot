...
public class BlockTBLeaves extends BlockOldLeaf {
	...
	public boolean canEntityDestroy(IBlockAccess world, int x, int y, int z, Entity entity) {
		if (world.getBlockMetadata(x, y, z) % 8 == 3 && entity instanceof net.minecraft.entity.boss.EntityDragon) {
			return false;
		}
	...
	public void randomDisplayTick(World w, int x, int y, int z, Random rnd) {
		...
		if (w.getBlockMetadata(x, y, z) % 8 == 0) {
			w.spawnParticle("reddust", x + rnd.nextDouble(), y + rnd.nextDouble(), z + rnd.nextDouble(), 1.0D, 1.0D, 0.0D);
		}
		if (w.getBlockMetadata(x, y, z) % 8 == 1 && rnd.nextFloat() <= 0.01F) {
			w.spawnParticle("heart", x + rnd.nextDouble(), y + rnd.nextDouble(), z + rnd.nextDouble(), 0.0D, 10.0D, 0.0D);
		}
		if (w.getBlockMetadata(x, y, z) % 8 == 2)
		{
			if (w.isAirBlock(x, y - 1, z)) {
				w.spawnParticle("dripLava", x + rnd.nextDouble(), y, z + rnd.nextDouble(), 0.0D, 0.0D, 0.0D);
			}
		}
	...
	public void updateTick(World w, int x, int y, int z, Random rnd) {
		...
		if (w.getBlockMetadata(x, y, z) % 8 == 1) {
			...
			if (rnd.nextInt(101) >= TBConfig.peacefulLeafSpawnChance) {
				return;
			}
			int dy = y;
			...
			if (base != null) {
				...
				List<BiomeGenBase.SpawnListEntry> l = base.getSpawnableList(EnumCreatureType.creature);
				if (l != null && !l.isEmpty()) {

					BiomeGenBase.SpawnListEntry entry = l.get(rnd.nextInt(l.size()));
					if (entry != null && entry.entityClass != null) {
						...
						if (EntityLiving.class.isAssignableFrom(c)) {
							...
							try {
								...
								EntityLiving el = EntityLiving.class.cast(c.getConstructor(new Class[] { World.class }).newInstance(new Object[] { w }));
								while (--y >= dy - 6) {

									el.setPositionAndRotation(x + 0.5D, y, z + 0.5D, 0.0F, 0.0F);
									if (el.getCanSpawnHere()) {

										w.spawnEntityInWorld((Entity)el);
		...
		if (w.getBlockMetadata(x, y, z) % 8 == 2) {
			...
			while (--y >= dy - 8) {
				...
				if (!b.isAir((IBlockAccess)w, x, y, z)) {

					boolean netheric = (b instanceof net.minecraft.block.BlockNetherrack || b instanceof net.minecraft.block.BlockSoulSand || b == Blocks.quartz_ore);
					if (netheric && rnd.nextInt(101) <= TBConfig.netherLeafSpawnChance) {

						BiomeGenBase hellBiome = BiomeGenBase.hell;

						List<BiomeGenBase.SpawnListEntry> l = rnd.nextBoolean() ? hellBiome.getSpawnableList(EnumCreatureType.creature) : hellBiome.getSpawnableList(EnumCreatureType.monster);
						if (l != null && !l.isEmpty()) {
							...
							if (entry != null && entry.entityClass != null) {
								...
								if (EntityLiving.class.isAssignableFrom(c)) {
									...
									try {
										...
										el.setPositionAndRotation(x + 0.5D, (y + 1), z + 0.5D, 0.0F, 0.0F);
					...
					boolean flag = (b instanceof net.minecraft.block.BlockDirt || b instanceof net.minecraft.block.BlockGrass || b instanceof net.minecraft.block.BlockGravel || b instanceof net.minecraft.block.BlockSand || b instanceof net.minecraft.block.BlockStone);
					if (!flag) {
		...
		if (w.getBlockMetadata(x, y, z) % 8 == 3) {
			...
			while (--y >= dy - 11) {
				...
				if (!b.isAir((IBlockAccess)w, x, y, z)) {
					...
					boolean end = (b == Blocks.end_stone || b instanceof net.minecraft.block.BlockObsidian);
					if (end && rnd.nextInt(101) <= TBConfig.enderLeafSpawnChance) {
					...
					boolean flag = (b instanceof net.minecraft.block.BlockDirt || b instanceof net.minecraft.block.BlockGrass || b instanceof net.minecraft.block.BlockGravel || b instanceof net.minecraft.block.BlockSand || (b instanceof net.minecraft.block.BlockStone && !(b instanceof net.minecraft.block.BlockObsidian) && b != Blocks.end_stone));
					if (!flag) {
	...
	protected void func_150124_c(World w, int x, int y, int z, int meta, int chance) {
		if (meta == 0 && w.rand.nextInt(chance) == 0) {
			dropBlockAsItem(w, x, y, z, new ItemStack(Items.golden_apple, 1, 0));
		}
	}
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < icons.length; i++) {
			icons[i] = reg.registerIcon("thaumicbases:" + textures[i]);
		}
