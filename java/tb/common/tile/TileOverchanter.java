...
public class TileOverchanter extends TileEntity implements IInventory, IWandable {
		...
		if (this.inventory == null) {
			...
			if (this.worldObj.getWorldTime() % 20L == 0L) {
				...
				if (EssentiaHandler.drainEssentia(this, Aspect.MAGIC, ForgeDirection.UNKNOWN, 8, false)) {
					...
					this.enchantingTime++;
					if (this.enchantingTime >= (TBCore.isDev ? 1 : 16) && !this.xpAbsorbed) {
					...
					if (this.xpAbsorbed && this.enchantingTime >= (TBCore.isDev ? 2 : 32)) {
	...
	public int findEnchantment(ItemStack enchanted) {
		...
		while ($i.hasNext()) {
			...
			Boolean capBoolean = (TBConfig.overchantCap ? ench.get(i) <= (Enchantment.enchantmentsList[i].getMaxLevel()) : true);
			
			if (TBCore.isDev && !this.worldObj.isRemote) {
				DummySteele.sendMessageFromServer("Enchantment name: " + StatCollector.translateToLocal(Enchantment.enchantmentsList[i].getName()));
				DummySteele.sendMessageFromServer("Enchantment id: " + i);
				DummySteele.sendMessageFromServer("Current Level: " + ench.get(i));
				DummySteele.sendMessageFromServer("Max level: " + Enchantment.enchantmentsList[i].getMaxLevel());
				DummySteele.sendMessageFromServer("Overchanter cap config: " + TBConfig.overchantCap);
				DummySteele.sendMessageFromServer("Cap boolean: " + capBoolean);
				DummySteele.sendMessageFromServer("-------------------------------------------");
			}

			
			if (!stackTag.hasKey("overchants") && !DummySteele.arrayContains(TBConfig.overchanterBlacklist, i) && capBoolean) {
				return i;
			}

			int[] overchants = stackTag.getIntArray("overchants");
			if (DummySteele.arrayContains(overchants, i) || DummySteele.arrayContains(TBConfig.overchanterBlacklist, i) || !capBoolean) {
				continue;
			}
			return i;
		}
	...
	public int findEnchantmentWithCap(ItemStack enchanted) {
