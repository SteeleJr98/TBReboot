	...
	public ContainerRevolver(InventoryPlayer inv, World w, int ix, int iy, int iz) {
		...
		if (!w.isRemote) {
			...
			try {
				...
				ItemStack jar = ItemStack.loadItemStackFromNBT(this.stack.stackTagCompound.getCompoundTag("jar"));
	...
	public void onContainerClosed(EntityPlayer p) {
		if (!this.worldObj.isRemote) {
			...
			if (var3 != null) {
				...
				this.stack.setTagInfo("jar", (NBTBase)var4);
				this.stack.getTagCompound().setBoolean("hasJar", true);
			}
			else {

				this.stack.setTagInfo("jar", (NBTBase)new NBTTagCompound());
