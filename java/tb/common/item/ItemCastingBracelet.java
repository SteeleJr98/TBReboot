...
public class ItemCastingBracelet extends ItemWandCasting {
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer player) {
		...
		if (focus != null && !isOnCooldown((EntityLivingBase)player)) {
			...
			WandManager.setCooldown((EntityLivingBase)player, focus.getActivationCooldown(getFocusItem(itemstack)) / 2);
	...
	public boolean isOnCooldown(EntityLivingBase base) {
		try {
			...
			Method method = manager.getDeclaredMethod("isOnCooldown", new Class[] { EntityLivingBase.class });
			boolean access = method.isAccessible();
			if (!access) {
				method.setAccessible(true);
			}
			boolean ret = ((Boolean)Boolean.class.cast(method.invoke((Object)null, new Object[] { base }))).booleanValue();
	...
	public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
		...
		if (wandable != null) {
			...
			if (focus != null && !isOnCooldown((EntityLivingBase)player)) {
				...
				WandManager.setCooldown((EntityLivingBase)player, focus.getActivationCooldown(getFocusItem(stack)) / 2);
