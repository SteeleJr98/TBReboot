 ...
 public class ItemConcentratedTaint extends Item {
   public boolean onItemUse(ItemStack stk, EntityPlayer user, World w, int x, int y, int z, int side, float vecX, float vecY, float vecZ) {
     stk.stackSize--;
     w.playSound(x, y, z, "break.glass", 1.0F, 1.0F, false);
     ...
     if (b == ConfigBlocks.blockAiry) {
       ...
       if (tile instanceof INode) {
         ...
         ((INode)INode.class.cast(tile)).setNodeType(NodeType.TAINTED);
         return true;
       } 
     } 
   ...
   public boolean itemInteractionForEntity(ItemStack stk, EntityPlayer user, EntityLivingBase usedOn) {
     if (!(usedOn instanceof thaumcraft.api.entities.ITaintedMob)) {
         ...
         for (int i = 0; i < victum.inventory.armorInventory.length; i++) {
           ...
           if (armor != null) {
             ...
             if (!victum.worldObj.isRemote) {
               victum.worldObj.spawnEntityInWorld((Entity)actualArmor);
             }
             armor = null;
   ...
   public ItemStack onEaten(ItemStack stack, World w, EntityPlayer player) {
     ...
     if (player.getHealth() < player.getMaxHealth()) {
       ...
       for (int i = 0; i < 99; i++) {
         player.attackEntityFrom(DamageSourceThaumcraft.taint, 2.14748365E9F);
       }
     } else {
       ...
       if (player.posY > 6.0D) {
         ...
         player.addChatMessage((new ChatComponentText(StatCollector.translateToLocal("tb.txt.howlBelow"))).setChatStyle((new ChatStyle()).setColor(EnumChatFormatting.DARK_PURPLE).setItalic(Boolean.valueOf(true))));
         TBUtils.addWarpToPlayer(player, 6, 0);
       
       }
       else if (player.worldObj.provider != null && player.worldObj.provider.dimensionId == -1) {
         
         player.addChatMessage((new ChatComponentText(StatCollector.translateToLocal("tb.txt.taintDevelop"))).setChatStyle((new ChatStyle()).setColor(EnumChatFormatting.DARK_PURPLE).setItalic(Boolean.valueOf(true))));
         ...
         if (!ResearchManager.isResearchComplete(player.getCommandSenderName(), "TB.TaintProgress")) {
           PacketHandler.INSTANCE.sendTo((IMessage)new PacketResearchComplete("@TB.TaintProgress"), (EntityPlayerMP)player);
         }
       } else {
         player.addChatMessage((new ChatComponentText(StatCollector.translateToLocal("tb.txt.howlDeeper"))).setChatStyle((new ChatStyle()).setColor(EnumChatFormatting.DARK_PURPLE).setItalic(Boolean.valueOf(true))));
