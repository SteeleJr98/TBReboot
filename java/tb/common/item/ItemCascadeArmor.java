...
public class ItemCascadeArmor extends ItemArmor implements IRepairable, IVisDiscountGear, IRunicArmor {

	int type;
	
	public ItemCascadeArmor(ItemArmor.ArmorMaterial material, int type) {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
		...
		list.add(EnumChatFormatting.GOLD + StatCollector.translateToLocal("item.runic.charge") + ": " + getRunicCharge(stack));
		super.addInformation(stack, player, list, par4);
	}

	static final int[] charge = new int[] {3, 2, 1, 0};
	
	@Override
	public int getRunicCharge(ItemStack itemstack) {
		return charge[this.type];
	}
	
	static final int[] discount = new int[] {7, 6, 5, 4};
