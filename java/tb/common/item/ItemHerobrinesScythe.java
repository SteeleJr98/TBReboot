...
public class ItemHerobrinesScythe extends ItemSword implements IRepairable, IWarpingGear {
	...
	private boolean hasWarnedPlayer = false;
	private int timeUntilWarn = TBConfig.warnTimer * 20;
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List lst, boolean held) {
		String[] edgy = {"The harvest waits in the shadows...", "Its edge hums with forgotten whispers...", "Every swing draws you closer to the void...", "Not all that it reaps can be returned...", "Darkness gathers where its blade falls...", "A silent promise of the end...", "The blade whispers, but only to you...", "The air chills where its edge lingers...", "Its secrets are boundless, its hunger eternal..."};
		lst.add(EnumChatFormatting.ITALIC + edgy[((int) ((player.worldObj.getWorldTime() / 1000) + player.experienceTotal + Thaumcraft.proxy.getPlayerKnowledge().getWarpTotal(player.getCommandSenderName())) % edgy.length)]);
		if (TBCore.isDev) {
			lst.add("warn timer: " + this.timeUntilWarn);
		}
	}
	...
	public void onUpdate(ItemStack stk, World w, Entity entity, int slot, boolean held) {
		...
		if (entity instanceof EntityPlayer) {
			...
			if (player.getCurrentEquippedItem()!= null && player.getCurrentEquippedItem().getItem() == this) {
				if (this.timeUntilWarn > 0) {
					this.timeUntilWarn--;
				}
				else {
					if (!this.hasWarnedPlayer) {
						if (TBUtils.checkMobBehindPlayer(w, player, TBConfig.warnRange)) {
							player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.DARK_PURPLE + "" + EnumChatFormatting.ITALIC + StatCollector.translateToLocal("tb.txt.scythewarn." + String.valueOf(w.rand.nextInt(5)))));
				...
				this.timeUntilWarn = TBConfig.warnTimer * 20;
