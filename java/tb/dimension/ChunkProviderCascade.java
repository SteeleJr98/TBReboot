...
public class ChunkProviderCascade implements IChunkProvider {
    ...
    private long provSeed = 0;
    
    float dimScale = 1.4F;
    int noiseScale = 1;

    public ChunkProviderCascade(World world, long seed, boolean genStruct)
    {
        this.worldObj = world;
        this.mapFeaturesEnabled = genStruct;
        if (this.provSeed == 0) {
        	this.provSeed = seed;
        }
        this.worldType = world.getWorldInfo().getTerrainType();
        this.rand = new Random(provSeed);
        this.noiseGen1 = new NoiseGeneratorOctaves(this.rand, 16 / noiseScale);
        this.noiseGen2 = new NoiseGeneratorOctaves(this.rand, 16 / noiseScale);
        this.noiseGen3 = new NoiseGeneratorOctaves(this.rand, 8 / noiseScale);
        this.noiseGen4 = new NoiseGeneratorPerlin(this.rand, 4 / noiseScale);
        this.noiseGen5 = new NoiseGeneratorOctaves(this.rand, 10 / noiseScale);
        this.noiseGen6 = new NoiseGeneratorOctaves(this.rand, 16 / noiseScale);
    ...
    public void genColumnData(int chunkX, int chunkY, Block[] blockArray)
    {
        byte b0 = 0; //water level?
        ...
        this.applyNoiseLayer(chunkX * 4, 0, chunkY * 4);
            ...
            for (int j1 = 0; j1 < 4; ++j1)
            {
                ...
                for (int k2 = 0; k2 < 32; ++k2) //8 = 32
                {
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        {
                            int nextBlock = i3 + k * 4 << 12 | 0 + j1 * 4 << 8 | k2 * 8 + l2;
                            ...
                            for (int k3 = 0; k3 < 4; ++k3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    blockArray[nextBlock += worldHeight] = Blocks.stone;
                                }
                                
                                else
                                {
                                    blockArray[nextBlock += worldHeight] = null;
                                }
                            }
    ...
    public Chunk provideChunk(int chunkX, int chunkY)
    {
        ...
        this.genColumnData(chunkX, chunkY, ablock);
...
//        }
    ...
    private void applyNoiseLayer(int chunkX, int chunkY, int chunkZ)
    {
        double d0 = 684.412D;
        double d1 = 684.412D;
        double d2 = 512.0D;
...
//        this.noiseData6 = this.noiseGen6.generateNoiseOctaves(this.noiseData6, chunkX, chunkZ, 5, 5, 200.0D, 200.0D, 0.5D);
//        this.noiseData3 = this.noiseGen3.generateNoiseOctaves(this.noiseData3, chunkX, chunkY, chunkZ, 5, 33, 5, 8.555150000000001D, 4.277575000000001D, 8.555150000000001D);
//        this.noiseData1 = this.noiseGen1.generateNoiseOctaves(this.noiseData1, chunkX, chunkY, chunkZ, 5, 33, 5, 684.412D, 684.412D, 684.412D);
//        this.noiseData2 = this.noiseGen2.generateNoiseOctaves(this.noiseData2, chunkX, chunkY, chunkZ, 5, 33, 5, 684.412D, 684.412D, 684.412D);

        this.noiseData6 = this.noiseGen6.generateNoiseOctaves(this.noiseData6, chunkX, chunkZ, 5, 5, 200.0D, 200.0D, 0.5D);
        this.noiseData3 = this.noiseGen3.generateNoiseOctaves(this.noiseData3, chunkX, chunkY, chunkZ, 5, 33, 5, 1.1D, 1.1D, 1.1D);
        this.noiseData1 = this.noiseGen1.generateNoiseOctaves(this.noiseData1, chunkX, chunkY, chunkZ, 5, 33, 5, 100D, 100D, 100D);
        this.noiseData2 = this.noiseGen2.generateNoiseOctaves(this.noiseData2, chunkX, chunkY, chunkZ, 5, 33, 5, 100D, 100D, 100D);
            ...
            for (int k1 = 0; k1 < 5; ++k1)
            {
                ...
                for (int l1 = -b0; l1 <= b0; ++l1)
                {
                    for (int i2 = -b0; i2 <= b0; ++i2)
                    {
                        ...
                        float f3 = biomegenbase1.rootHeight * dimScale;
                ...
                double d12 = this.noiseData6[i1] / 8000.0D;
                ...
                for (int j2 = 0; j2 < 33; ++j2)
                {
                    ...
                    double d7 = this.noiseData1[l] / 512.0D;
                    double d8 = this.noiseData2[l] / 512.0D;
                    double d9 = (this.noiseData3[l] / 10.0D + 1.0D) / 2.0D;
